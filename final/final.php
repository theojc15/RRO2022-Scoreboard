...
<body>
	...
	<div class="collapse navbar-collapse" id="navbarNav">
		...
		<li class="nav-item active">
			<a class="nav-link" href="final.php">Final</a>
		</li>
		<li class="nav-item">
					...
					<table>
						<thead>
							<tr class="table100-head">
								...
								<th class="column2">Take Point</th>
						...
						<tbody>
							<?php
								$n = 1;
								$tampil = mysqli_query($connection, "SELECT * FROM final INNER JOIN peserta on final.kode = peserta.kode ORDER BY tag DESC, time ASC");
								while($data = mysqli_fetch_array($tampil)) {
							?>
							<tr>
								<td class="column1"><?=$n++?></td>
								<td class="column3"><?=$data['nama']?></td>
								<td class="column2"><?=$data['tag']?></td>
								<td class="column2"><?=$data['time']?></td>
							</tr>
							<?php } ?>
						</tbody>
					</table>
